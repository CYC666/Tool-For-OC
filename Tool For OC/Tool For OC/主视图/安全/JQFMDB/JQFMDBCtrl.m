...
@interface JQFMDBCtrl ()

@end

@implementation JQFMDBCtrl
...
- (void)viewDidLoad {
    ...
    ProductionView *proViews = [[ProductionView alloc] initWithFrame:CGRectMake(0, Nav_Height, kScreenWidth, kScreenHeight - Nav_Height)];
...
- (void)lookupMethod:(ProductionView *)proViews db:(JQFMDB *)db model:(Person *)person array:(NSMutableArray *)mArr
{
    [proViews lookupMethod1:^{
        ...
        [showMessage showAlertWith:[NSString stringWithFormat:@"name=cleanmonkey:%@", personArr]];
    }];
    ...
    [proViews lookupMethod2:^{
        ...
        FadeAlertView *showMessage = [[FadeAlertView alloc] init];
        [showMessage showAlertWith:[NSString stringWithFormat:@"表中所有数据:%@", personArr]];
    }];
    ...
    [proViews lookupMethod3:^{
        ...
        [db jq_inDatabase:^{
            NSArray *personArr = [db jq_lookupTable:@"user" dicOrModel:[Person class] whereFormat:@"where name = 'cleanmonkey'"];
            FadeAlertView *showMessage = [[FadeAlertView alloc] init];
            [showMessage showAlertWith:[NSString stringWithFormat:@"(safe)name=cleanmonkey:%@", personArr]];
        }];
    }];
    ...
    [proViews lookupMethod4:^{
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            NSArray *personArr = [db jq_lookupTable:@"user" dicOrModel:[Person class] whereFormat:@"where name = 'cleanmonkey'"];
            FadeAlertView *showMessage = [[FadeAlertView alloc] init];
            [showMessage showAlertWith:[NSString stringWithFormat:@"(async)name=cleanmonkey:%@", personArr]];
