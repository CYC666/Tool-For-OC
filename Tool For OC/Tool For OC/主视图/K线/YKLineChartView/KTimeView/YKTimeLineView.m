...
- (void)drawTimeLabel:(CGContextRef)context
{
    ...
    if (self.countOfTimes == 242||self.countOfTimes==332) { // 分时与五日
        //沪深分时 和  港股分时
        ...
        [self drawLabel:context
         attributesText:stopTimeAttStr
                   rect:CGRectMake(self.contentRight -sizeStopTimeAttStr.width, (self.uperChartHeightScale * self.contentHeight+self.contentTop), sizeStopTimeAttStr.width, sizeStopTimeAttStr.height)];
    }else{
