...
- (void)buildView {
    ...
    if (iPhone4_4s) {
        ...
        // 显示控件
        self.weatherIconView = [[WeatherIconView alloc] initWithFrame:CGRectMake(0, headerHeight, Width / 2.f, Width / 2.f)];
        [self.weatherIconView buildView];
        [self.tableView addSubview:self.weatherIconView];
        
        self.temperatureView = [[TemperatureView alloc] initWithFrame:CGRectMake(Width / 2.f, headerHeight, Width / 2.f, Width / 2.f)];
        [self.temperatureView buildView];
        [self.tableView addSubview:self.temperatureView];
        
        self.humidityView = [[HumidityView alloc] initWithFrame:CGRectMake(0, headerHeight + Width / 2.f, Width / 2.f, Width / 2.f)];
        [self.humidityView buildView];
        [self.tableView addSubview:self.humidityView];
        
        self.sunInfoView = [[SunInfoView alloc] initWithFrame:CGRectMake(Width / 2.f, headerHeight + Width / 2.f, Width / 2.f, Width / 2.f)];
        [self.sunInfoView buildView];
        [self.tableView addSubview:self.sunInfoView];
        
        self.maxTempView = [[MaxTempView alloc] initWithFrame:CGRectMake(0, headerHeight + Width, Width / 2.f, Width / 2.f)];
        [self.maxTempView buildView];
        [self.tableView addSubview:self.maxTempView];
        
        self.windSpeedView = [[WindSpeedView alloc] initWithFrame:CGRectMake(Width / 2.f, headerHeight + Width, Width / 2.f, Width / 2.f)];
    ...
    // 创建出线条
    {
        self.grayLine1                 = [[LeftToRightView alloc] initWithFrame:CGRectMake(0, headerHeight, Width, 0.5)];
        self.grayLine1.backgroundColor = [UIColor blackColor];
        self.grayLine1.alpha           = 0.1;
        [self.tableView addSubview:self.grayLine1];
        
        self.grayLine2                 = [[LeftToRightView alloc] initWithFrame:CGRectMake(0, headerHeight + Width / 2.f, Width, 0.5)];
        self.grayLine2.backgroundColor = [UIColor blackColor];
        self.grayLine2.alpha           = 0.1;
        [self.tableView addSubview:self.grayLine2];
        
        self.grayLine3                 = [[LeftToRightView alloc] initWithFrame:CGRectMake(0, headerHeight + Width, Width, 0.5)];
        self.grayLine3.backgroundColor = [UIColor blackColor];
        self.grayLine3.alpha           = 0.1;
        [self.tableView addSubview:self.grayLine3];
        
        self.grayLine4                 = [[LeftToRightView alloc] initWithFrame:CGRectMake(0, headerHeight + Width * 1.5, Width, 0.5)];
        self.grayLine4.backgroundColor = [UIColor blackColor];
        self.grayLine4.alpha           = 0.1;
        [self.tableView addSubview:self.grayLine4];
        
        self.vLine                     = [[UpToDownView alloc] initWithFrame:CGRectMake(Width / 2.f - 1, headerHeight, 0.5, Width + Width / 2.f)];
    ...
    // 标题
    self.cityTitleView = [[CityTitleView alloc] initWithFrame:CGRectMake(0, 0, Width, headerHeight)];
